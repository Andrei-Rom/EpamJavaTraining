...
public class CustomArrays {
    ...
    private static boolean binarySearch(int[] input, int number) {
    ...
    public int getMax(CustomArray array) {
    ...
    public int getMin(CustomArray array) {
        int[] arr = array.getArray();
        int min = arr[0];
        for (int i = 1; i < arr.length; i++) {
            if (arr[i] < min) {
    ...
    public CustomArray getFibonacciNumbers(CustomArray array) {
        CustomArray customArray = new CustomArray();
        int max = new CustomArrays().getMax(array);
        int[] arr = array.getArray();
